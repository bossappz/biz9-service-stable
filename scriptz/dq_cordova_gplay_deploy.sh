...
#apk
${ZIPALIGN_DIR}/zipalign -f -v 4  ${APP_DEBUG_APK} ${APP_TITLE_ID}_${APP_VERSION_NEW}".apk"
apksigner sign --ks-key-alias alias_name --ks ${KEY_STORE} --ks-pass pass:"${KEY_STORE_PASSWORD}" --min-sdk-version 22 ${APP_TITLE_ID}_${APP_VERSION_NEW}".apk"
...
#aab
${ZIPALIGN_DIR}/zipalign -f -v 4  ${APP_BUNDLE_RELEASE} ${APP_TITLE_ID}_${APP_VERSION_NEW}".aab"
apksigner sign --ks-key-alias alias_name --ks ${KEY_STORE} --ks-pass pass:"${KEY_STORE_PASSWORD}"  --min-sdk-version 22 ${APP_TITLE_ID}_${APP_VERSION_NEW}".aab"
...
java -jar /home/mama/www/opz/toolz/bundletool/bundletool-all-1.9.1.jar build-apks --bundle=${APP_TITLE_ID}_${APP_VERSION_NEW}'.aab' --output=${APP_TITLE_ID}_${APP_VERSION_NEW}'.apks' --ks=${KEY_STORE} --ks-key-alias=alias_name --ks-pass=pass:${KEY_STORE_PASSWORD}
echo "BiZ9 MOBILE BUNDLETOOL OK..."
rsync -rave "ssh -2 -i ${AWS_KEY_PEM}" ${APP_TITLE_ID}_${APP_VERSION_NEW}.apk  admin@${DEPLOY_IP}:${DEPLOY_LOC}
rsync -rave "ssh -2 -i ${AWS_KEY_PEM}" ${APP_TITLE_ID}_${APP_VERSION_NEW}.aab  admin@${DEPLOY_IP}:${DEPLOY_LOC}
echo "---------------------------------------------"
echo "#################"
